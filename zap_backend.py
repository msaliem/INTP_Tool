...
from jsonData import  write_jsonForZap
import tkinter
...
def calculateMinMaxAvg(timeElapse,deltaTimeWithDefaultStep,minMaxAvgArr):
    ...
    for i in range(0,len(deltaTimeWithDefaultStep[0])):
         ...
         try:
             ...
             row.append(round(mx,3))
             row.append(round(mn,3))
             row.append(round(float(sumOfData/len(deltaTimeWithDefaultStep)),3))
    ...
    try:
        ...
        #print(sumOfData         
        row1.append(round(mx,3))
        row1.append(round(mn,3))
        row1.append(round(float(sumofdata/len(timeElapse)),3))
        print(row1)
...
def zapTimeCal(timeElapseWithDefaultStep,timeElapseforCA,deltaTimeWithDefaultStep,deltaTimeWithCA,Data,minMaxAvgforDefaultStep,minMaxAvgforCA,MWsequence,defaultStepsDetail,CA_SequnceStepsDetail,MW_SequnceStepsDetail,Totalzaptime,channnel_num,filename,useCase):
    ...
    dic={}
    def fun(indexTocalculateDefaultTime,indexToCalculateCA,pattern,string,channnel_num):
        row=[]
        boolSource=True
        boolDesti=True
        
        for line in string.splitlines():
            #if 'original lcn= '+str(channnel_num[0]) in line or 'original LCN= '+str(channnel_num[0]) in line:
                #boolSource=True   
               
            #if 'original lcn= '+str(channnel_num[1]) in line or 'original LCN= '+str(channnel_num[1]) in line:
               # boolDesti=True
                
               

              
            if boolSource==True and boolDesti==True:
               #print(line)
            ...
            if pattern[-1] in line:
                ...
                for i in range(1,len(indexTocalculateDefaultTime)):
                    try:
                       res1.append(round(float(dic[indexTocalculateDefaultTime[i]]-dic[pattern[0]])/1000,3))
                       res.append(round(float(dic[indexTocalculateDefaultTime[i]]-dic[indexTocalculateDefaultTime[i-1]])/1000,3))
                    except:
                        boolDefault=True
                        break
                ...
                for i in range(1,len(indexToCalculateCA)):
                    try:
                        resCca1.append(round(float(dic[indexToCalculateCA[i]]-dic[pattern[0]])/1000,3))
                        resCca.append(round(float(dic[indexToCalculateCA[i]]-dic[indexToCalculateCA[i-1]])/1000,3))
                    except:
                        boolCca=True
                        break
                #print(resCca)
    ...
    projectName=filename.split('_')[2]
    print("hello.....")
    """
    ...
    totalZapTimeMx=max(Totalzaptime)
    totalZapTimeMn=min(Totalzaptime)
    totalZapSum=(sum(Totalzaptime))/len(Totalzaptime)
    row=[]
    ...
    row.append(round(totalZapTimeMx,3))
    row.append(round(totalZapTimeMn,3))
    row.append(round(totalZapSum,3))
    MWsequence.append(row)
    print("MinMax Avg cal.....")
    jsonDictionary={}
    jsonDictionaryUsecase={}
    ...
    for i in range(len(minMaxAvgforDefaultStep)-1):
        ...
        print("\n-----------------------------------------------------------------------------------\n")
        jsonDictionary[defaultStepsDetail[i]]=[minMaxAvgforDefaultStep[i][0],minMaxAvgforDefaultStep[i][1],minMaxAvgforDefaultStep[i][2]]
    for i in range(len(minMaxAvgforCA)-1):
        ...
        print("-----------------------------------------------------------------------------------\n")
        jsonDictionary[CA_SequnceStepsDetail[i]]=[minMaxAvgforCA[i][0],minMaxAvgforCA[i][1],minMaxAvgforCA[i][2]]

    print("\n MW start sequence\n")
    for i in range(len(MWsequence)):
         print(MW_SequnceStepsDetail[i],"; ",MWsequence[i][0],"; ",MWsequence[i][1],"; ",MWsequence[i][2])
         jsonDictionary[MW_SequnceStepsDetail[i]]=[MWsequence[i][0],MWsequence[i][1],MWsequence[i][2]]
...
#    print("\niteration",i+1,"Done ____________________________\n")
    ...
    if len(jsonDictionary)>0:
       write_jsonForZap(jsonDictionary,projectName,useCase)
       return True
    else:
       return False
